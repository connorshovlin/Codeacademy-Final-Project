...
#removed period from Michael B. Jordan to prevent later issues
...
def get_average_sentence_length(text):
    #replaces question marks and exclamation points with periods
    text_no_excl = text.replace('!', '.')
    new_text = text_no_excl.replace('?', '.')
    individual_sentences = new_text.split('. ')
    individual_words = []
    for sentence in individual_sentences:
        words = sentence.split(' ')
        for word in words:
            individual_words.append(word)
    num_sentences = len(individual_sentences)
...
def prepare_text(text):
    ...
    unpunctuated_text1 = lowered_text.replace('!', '')
    unpunctuated_text2 = unpunctuated_text1.replace('?', '')
    unpunctuated_text3 = unpunctuated_text2.replace('.', '')
    unpunctuated_text4 = unpunctuated_text3.replace(';', '')
    unpunctuated_text = unpunctuated_text4.replace(',', '')
    final_text = unpunctuated_text.split(' ')
    words_in_order = []
...
def ngram_creator(text_list):
    ...
    for i in range(0, len(text_list) - 1): 
        output.append(text_list[i] + ' ' + text_list[i+1])
    return output
         
#tests the function
#print(ngram_creator(prepare_text(murderer_note)))
...
Average sentence length: {length}""".format(author = self.author, length = self.average_sentence_length)


# In[237]:


murderer_sample = TextSample(murderer_note, 'Murderer')
...
def percent_difference(value1, value2):
    ...
    #I separated them so I could see he equation more clearly
...
for contestant in contestants:
    print("Name: {name}; Similarity Score: {score}".format(name = contestant.author, score = find_text_similarity(contestant, murderer_sample)))
